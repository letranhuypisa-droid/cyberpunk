...
/* =====================================================================
   ...
   Mặc định: localStorage. Muốn lưu server: đặt SAVE.mode='remote' và trỏ SAVE.endpoint
   tới API nhận PUT/GET JSON hồ sơ (chưa có backend — xem docs/story.md mục 6).
...
function dailyProgress(id, n=1){
  ...
  if(before<t.goal && d.prog[id]>=t.goal && window.AUDIO) AUDIO.ready();
  savePlayer();
}
