...
Mỗi nhân vật có đúng một chiêu cuối, và khi phát chiêu thì trận dừng lại để chạy một video
cut-in 5 giây (`js/battle.js` → `playCutin`). Đây là chỗ tốn tiền nhất của game về mặt cảm giác:
...
Lấy từ `css/chromefall.css` (`.cutin`) và `js/battle.js`, không phải ước lượng:

| Mục | Giá trị | Vì sao |
|---|---|---|
| Tỉ lệ | **16:9** | `.cutin video{aspect-ratio:16/9}` |
| Độ phân giải nộp vào repo | **1280×720** H.264 + AAC | bằng `kira_ult.mp4` đang chạy |
| Thời lượng | **5.0s** (tối đa 8s) | quá 8s bị `guard` cắt để không treo trận |
| Vùng an toàn ngang | **80% giữa khung** | video kéo `width:125%` + `object-fit:cover` → cắt ~10% mỗi mép |
| Vùng an toàn dọc | toàn bộ | dải đen chéo trên/dưới nằm ngoài video, không đè lên |
| Chữ trong video | **không** | tên chiêu (`ZERO`) do CSS vẽ đè lên, chèn nữa là chồng nhau |
| Watermark | **tắt** | `kira_ult.mp4` hiện tại còn dính "Dola AI" ở góc phải dưới |
| Frame đầu | đã sáng sẵn, vào thẳng cao trào | CSS `cutin-in` chỉ có 260ms, không có chỗ cho intro |
...
Tên file: `<id>_ult.mp4` đặt cùng thư mục `index.html`. Chín nhân vật đã có art: `kira`, `kai`, `ash`,
`psalm`, `ronin`, `muzzle`, `echo`, `wire`, `stitch` — cả chín đã trỏ sẵn `ultVideo` trong `js/data.js`.

Hạ về chuẩn repo sau khi generate:

```bash
ffmpeg -i raw.mp4 -vf scale=1280:720 -c:v libx264 -profile:v high -pix_fmt yuv420p -crf 23 -c:a aac -b:a 96k -movflags +faststart kai_ult.mp4
```
...
**Nên chạy image-to-video**, lấy đúng file art của nhân vật (`KIRA.png`, `Kai.png`, `ASH.png`,
`PSALM.png`, `RONIN.png`, `MUZZLE.png`, `ECHO.png`, `WIRE.png`, `STITCH.png`) làm ảnh tham chiếu
...
|---|---|---|
| KIRA | orbit 180° quanh người | cô di chuyển, đối thủ thì không |
...
- **Kira đứng trước một Kira khác.** Prompt tả Kira rất kỹ, còn mục tiêu chỉ gọi là
...
`no second Kira, no clone, no twin, no double, no reflection, no mirrored copy`.
...
Model trôi qua mỗi lần cắt. Dán `Kira (white bob, cat-ear headgear, white haori)` vào đầu mỗi shot.
...
printf "file 's1.mp4'\nfile 's2.mp4'\nfile 's3.mp4'\nfile 's4.mp4'\n" > list.txt
ffmpeg -f concat -safe 0 -i list.txt -vf scale=1280:720 -c:v libx264 -profile:v high -pix_fmt yuv420p -crf 23 -c:a aac -b:a 96k -movflags +faststart kai_ult.mp4
```
...
```
=== VOICE — only Kira speaks. No narrator, no second voice ===
...
> Kira càng giết càng vui như đang chơi nhảy lò cò. Kai càng sợ bị quên càng gào tên mình. Ash nổ
...
Kira không lao tới. Cô đi ba bước.
...
Thanh kiếm về bao trước khi cái xác kịp đổ. Kira quay lưng đi, không nhìn lại. Vì sao ư? Vì với
...
[1] KIRA — a small pale girl, short white bob, white cat-ear headgear, half-lidded violet eyes,
...
FORBIDDEN: a second Kira, a clone, a twin, a double, her reflection, a mirrored copy, a
...
=== SHOTS ===
SHOT 1 — 0.0-1.0s
IN FRAME: Kira only (white bob, cat-ear headgear, white haori), the wet floor, the halo above her.
Nothing else.
...
SHOT 3 — 1.9-3.4s
IN FRAME: Kira only (white bob, white haori), the katana in her hands, empty black space around
...
SHOT 4 — 3.4-5.0s
IN FRAME: Kira from behind over her shoulder (white bob, white haori), the katana sliding into the
...
=== VOICE — only Kira speaks. No narrator, no second voice, no crowd ===
0.5s  "Hop!"    — bright girlish sing-song, delighted, weightless
...
Keep Kira inside the central 80% of the frame at all times. The first frame is already at peak
intensity.

=== DO NOT RENDER ===
No subtitles, no captions, no burned-in text, no letters, no logo, no watermark, no UI, no name
plate, no lens flare crosses, no second character, no clone, no reflection of Kira, no crowd,
no blood, no gore.
```
...
Khẩu súng của Kai to hơn Kai. Ghép từ ray điện tháo ở một toa hàng và một cái ống bơm dầu, nặng
tới mức cậu phải quỳ một chân xuống mới ghì nổi. Nó không có nút bấm. Nó có một sợi dây giật, y
hệt máy nổ dưới bãi.

Kai giật một cái. Cuộn từ trong nòng rú lên, cả nòng đỏ dần từ khoá nòng ra tới đầu, và cậu gào
đúng ba chữ trước khi bóp cò: K. A. I. Một phát. Chỉ một phát, vì nạp lại mất bốn mươi giây mà cậu
thì không có bốn mươi giây bao giờ.

Cậu gào làm gì ư? Vì Kai lớn lên ở trại trẻ tầng âm tám, nơi bị quên là chết lần thứ hai. Vỏ đạn
rơi xuống sàn, to bằng cổ tay, và Kai bao giờ cũng cúi nhặt. Cậu giữ hết. Cậu bảo sau này người ta
đếm lại thì phải cho đúng.

Gào xong, câu cuối cậu nói rất khẽ, và nói với chính mình.
...
Mìn đã nằm sẵn dưới sàn từ lúc trận đánh mới bắt đầu — cô đi qua chỗ nào là gài chỗ ấy, và cô nhớ
giá từng quả. Đến lượt cô, cô làm đúng ba việc: lật nắp kíp nổ, búng điếu thuốc xuống sàn, và quay
lưng lại.

Cả sàn bốc lên sau lưng cô cùng một lúc. Dân trong nghề gọi cái đó là flashover: không phải một
chỗ cháy rồi lan, mà mọi chỗ cùng chạm ngưỡng một lượt. Ash không quay đầu nhìn. Cô nhìn hai bàn
tay đầy sẹo bỏng của mình, thứ cô lấy về từ một đường hầm sập sớm hơn tính toán, với Muzzle còn
kẹt bên trong.

Cả chiêu cô không hề lên giọng lấy một lần.
...
Kira cũng rút kiếm. Nhưng Kira rút xong là đi, còn Ronin rút xong là đứng lại.
...
Cú máy phải khác Kira cho bằng được: **tuyệt đối không orbit**. Wide tĩnh, rồi snap dolly-in đúng
lúc chém.
...
Echo được xuất xưởng ba ngày sau khi Tài sản 07 rơi, mang dải giọng sao chép từ hồ sơ của Kira.
